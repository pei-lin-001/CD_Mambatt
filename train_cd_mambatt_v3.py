...
def resolve_inv_mmd_lambda(args: argparse.Namespace) -> float:
    ...
    # Legacy fallback for historical configs recorded before inv-MMD became the maintained path.
    return float(getattr(args, "lambda_domain_adv", 0.0))
        ...
        for _ in range(num_steps):
            ...
            if lambda_spec_domain > 0 and spec_domain_classifier is not None:
                ...
                if source_specific_features is not None and target_specific_features is not None:
                    ...
                    spec_domain_loss = ce_loss(spec_domain_classifier(spec_domain_features), spec_domain_labels)
                else:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="CD-MambAtt v3 / SPD-only: source supervision + target few-shot supervision + MMD/pseudo/monotonic scaffold + SPD-style DD-Mamba with optional invariant-path MMD alignment."
    )
    ...
    parser.add_argument("--inv-alignment-mode", choices=("mmd", "none"), default="mmd", help="Invariant-path alignment mode for SPD")
    parser.add_argument("--lambda-inv-mmd", type=float, default=0.1, help="Weight for SPD invariant-path MMD alignment")
